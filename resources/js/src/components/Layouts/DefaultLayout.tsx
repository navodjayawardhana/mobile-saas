...
const DefaultLayout = ({ children }: PropsWithChildren) => {
    ...
    // Update document title and favicon with shop data
    useEffect(() => {
        if (shopData?.shop?.name) {
            document.title = shopData.shop.name;
        }
        if (shopData?.shop?.logo) {
            const favicon = document.querySelector("link[rel='shortcut icon']") as HTMLLinkElement;
            if (favicon) {
                favicon.href = shopData.shop.logo;
            }
        }
    }, [shopData?.shop?.name, shopData?.shop?.logo]);
